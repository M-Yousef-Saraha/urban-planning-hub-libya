...
const RequestsManagement = () => {
  ...
  // Check admin access (wait for auth to finish loading to avoid false redirects)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!user || user.role !== 'ADMIN') return; // don't fetch if not admin
    fetchRequests();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [statusFilter, urgencyFilter, authLoading, user?.role]);
  ...
  const fetchRequests = async () => {
    try {
      ...
      if (error?.response?.status === 403) {
        setError('ليست لديك صلاحية الوصول إلى إدارة الطلبات (حساب غير إداري)');
  ...
  // Auth loading or redirect guard
