  ...
  <div className="flex justify-between items-center py-4 md:py-6">
          ...
          <div className="hidden lg:flex items-center flex-shrink-0 ps-4">
            <button 
              ...
              className={`flex items-center ${isRTL ? 'space-x-2 space-x-reverse' : 'space-x-2'} px-5 py-2.5 border-2 border-primary text-primary rounded-full hover:bg-primary hover:text-primary-foreground transition-all duration-300 font-medium text-sm md:text-base shadow-sm hover:scale-105 btn-micro`}
            >
          ...
          >
            {isMenuOpen ? (
              <X size={24} className="text-primary icon-bounce" />
            ) : (
              <Menu size={24} className="text-primary icon-bounce" />
            )}
          </button>
        </div>
        ...
        {isMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 right-0 z-[71] py-4 border-t border-primary/20 bg-white/95 backdrop-blur-md shadow-lg">
            <nav className="flex flex-col space-y-3 px-4">
              ...
              <button 
                onClick={toggleLanguage}
                className={`flex items-center justify-center ${isRTL ? 'space-x-2 space-x-reverse' : 'space-x-2'} px-4 py-3 border-2 border-primary text-primary rounded-full hover:bg-primary hover:text-primary-foreground transition-all duration-200 font-medium mt-4`}
              >
                <Globe size={16} />
                <span>{language === 'ar' ? t('language.english') : t('language.arabic')}</span>
              </button>
